...
\begin{proof}
If we suppose that $2^r = 3$ for some rational $r$, we can show that $2^p = 3^q$ for some $p, q \in \N$.
However, we can show that all powers of 2 are even whereas all powers of 3 are odd. Therefore, such a $p, q$
cannot exist.
\end{proof}
